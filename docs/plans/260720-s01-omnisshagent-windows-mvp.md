...
- 設定変更のホットリロードは行わない
- Windowsログオン時の自動起動登録はMVP後の追加機能としてタスクトレイから設定可能にする
...
- Windowsログオン時の自動起動はMVP後の追加機能としてタスクトレイから設定する
- タスクトレイ以外のGUIはない
...
- [x] `PREP-001` 要求仕様書を`docs/requirements/260720-omnisshagent-windows-mvp-requirements.md`へ追加する
- [x] `PREP-002` 本計画を`docs/plans/260720-s01-omnisshagent-windows-mvp.md`へ追加する
...
- [x] `TRAY-013` チェック付きメニューから現在のユーザーのWindowsログオン時自動起動を登録・解除する
- [x] `TRAY-014` アンインストール時に自動起動登録を削除する
- [x] `TRAY-015` TOMLのPageant、Cygwin/MSYS2のBoolean設定をチェック付きメニューから保存する
- [ ] `TRAY-FUTURE-001` 署名利用通知と`tray.show_sign_notifications`を接続する
...
- 日次ログの古いファイルを自動削除しない
- 設定変更には手動再起動が必要
- Windows自動起動はMVP後の追加機能としてタスクトレイから提供する
