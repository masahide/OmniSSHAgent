...
Run `go test ./...`, `go vet ./...`, `./scripts/test-installer.ps1`, and both
console and `windowsgui` builds before marking implementation tasks complete.
Windows-specific code must use a `//go:build windows` constraint. Never log SSH
agent payloads, private keys, passphrases, or signing data.
